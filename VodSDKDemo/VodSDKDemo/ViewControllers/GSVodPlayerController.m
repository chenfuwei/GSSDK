...
@property (nonatomic, strong) GSVodSectionsView *sectionView;
/**
 栏目选择bar
 */
...
- (void)viewDidLoad {
    ...
    _vodView.backgroundColor = [UIColor greenColor];
...
//    _videoArea.backgroundColor = [UIColor greenColor];
    ...
    _itemBar = [[GSItemBar alloc] initWithFrame:CGRectMake(0, _playerBar.bottom, Width, 40) style:@[@"文档",@"聊天",@"点播切换",@"章节"] event:^(int index) {
        switch (index) {
            case 0:{
            ...
            case 2:{
                ...
                break;
            case 3:{
                [wself.scrollView setContentOffset:CGPointMake(Width*3, 0)];
            }
                break;
            default:
                break;
...
//    //to fix autolayout case if you need
    ...
    //chatview
    _chatView = [[GSChatView alloc] initWithFrame:CGRectMake(Width, 0, Width, Height - CGRectGetMaxY(_itemBar.frame))];
    ...
    //sectionsview
    _sectionView = [[GSVodSectionsView alloc] initWithFrame:CGRectMake(Width*3, 0, Width, Height - CGRectGetMaxY(_itemBar.frame))];
    [_scrollView addSubview:_sectionView];
    
    
    GSTagsContentView *tagView = [[GSTagsContentView alloc] initWithFrame:CGRectMake(10, Height - 40, Width - 20, 30) tags:@[@"日志上传",@"下载"] handler:^(NSInteger index, NSString *text, BOOL isSelect) {
        if (index == 0) {
            ...
            GSVodQueueController *queue = [[GSVodQueueController alloc] init];
            [wself.navigationController pushViewController:queue animated:YES];
        }
    }];
...
//    [_vodPlayer seekTo:1668020];
    ...
    //doc章节
    NSLog(@"docinfo %@",docInfos);
    [_sectionView.dataModelArray addObjectsFromArray:docInfos];
    [_sectionView reloadView];
    //chat
...
- (void)refreshChatView {
    ...
    if (self.msgCount != self.chatView.dataModelArray.count) {
        self.itemBar.styles = @[@"文档",[NSString stringWithFormat:@"聊天(%d)",receiveChatCount],@"点播切换",@"章节"];
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        if (chatList.count > 0) {
            for (int i = 0; i < chatList.count; i ++) {
                ...
                GSChatModel *model = [[GSChatModel alloc] initWithModel:chatInfo];
