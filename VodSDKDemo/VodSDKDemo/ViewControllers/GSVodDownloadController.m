...
- (void)viewDidLoad {
    ...
    
    {
        ...
        start.tag = 0;
        [self resetCustomButton:start flag:1];
...
- (void)customAction:(UIButton *)sender {
    switch (sender.tag) {
        case 0:{ //开始下载
            [[GSVodManager sharedInstance] request:self.item download:YES completion:^(downItem *item, GSVodWebaccessError type) {
                NSString *msg;
                switch (type) {
                    case GSVodWebaccessSuccess:
                        {
                            ...
                            dispatch_async(dispatch_get_main_queue(), ^{
                                ...
                                [_vodID sizeToFit];
                                [_name sizeToFit];
                                [[GSVodManager sharedInstance] startDownload:item.strDownloadID];
                                [self resetCustomButton:_start  flag:0];
            ...
            break;
        case 1:{
            [[GSVodManager sharedInstance] stopDownload:_downloadingItem.strDownloadID];
        }
            break;
        case 2:{ //删除下载
        ...
        case 3:{ //离线观看
            ...
            player.isOnline = NO;
            player.isChatpost = YES;
//            [GSVodManager sharedInstance].isFlv = YES;
